...
type JsonInputValue = string | number | boolean | JsonObject | JsonInputValue[];
...
const DEFAULT_SETTINGS = [
  ...
  { key: 'site_logo', value: '', type: SettingType.IMAGE, label: 'Logo', group: 'general', sortOrder: 3 },
  { key: 'favicon', value: '', type: SettingType.IMAGE, label: 'Favicon', group: 'general', sortOrder: 4 },
...
function toJsonInputValue(value: unknown): JsonInputValue {
    ...
    Object.entries(updates).map(async ([key, value]) => {
      ...
      if (setting) {
        return prisma.setting.update({
          where: { key },
          data: { value: toJsonInputValue(value) },
        });
      }
      return null;
    })
